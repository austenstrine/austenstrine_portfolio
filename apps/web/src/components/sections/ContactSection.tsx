...
export function ContactSection() {
  return (
    <section id="contact" className="rounded-2xl bg-gradient-to-br from-ink to-sky p-8 text-white shadow-xl">
      <h2 className="text-3xl font-semibold">Get In Touch</h2>
      <p className="mt-3 max-w-3xl text-slate-200">
        I am always interested in hearing about new opportunities and projects. Whether you have a question or just
        ...
        <div className="grid gap-3">
          {methods.map((method) => (
            <a
              ...
              className="group flex items-center justify-between rounded-xl border border-white/20 bg-white/10 px-4 py-3 transition hover:-translate-y-0.5 hover:bg-white/20"
            >
              <div className="flex items-center gap-3">
                <span className="rounded-md bg-white/15 p-2">{method.icon}</span>
                <div>
                  <p className="font-semibold">{method.title}</p>
                  <p className="text-sm text-slate-200">{method.summary}</p>
                </div>
              </div>
